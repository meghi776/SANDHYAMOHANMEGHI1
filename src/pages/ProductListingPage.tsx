...
interface Product {
  ...
  mockup_image_url: string | null; // Added mockup_image_url
}
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      let query = supabase.from('products').select(`
        id, name, description, image_url, price, canvas_width, canvas_height, is_disabled, inventory, sku,
        mockups(image_url)
      `);
      let breadcrumbTitle = '';
      let backLink = '/';
      ...
      if (productsError) {
        ...
        setProducts(data.map(p => ({
          ...p,
          mockup_image_url: p.mockups?.[0]?.image_url || null, // Extract mockup_image_url
        })) || []);
      }
      setLoading(false);
    };
        ...
        <div className="flex items-center mb-6 justify-between">
          ...
          <div className="relative flex-1 min-w-0 ml-auto">
            ...
            <Input
              ...
              className="pl-9 pr-3 py-2 rounded-md border border-input bg-background shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring w-full"
            />
          </div>
        </div>
                ...
                {products.map((product) => (
                  <div key={product.id} className="w-full">
                    <Link
                      ...
                      className={`w-full h-auto py-2 px-4 text-base font-semibold transition-colors duration-200 flex flex-col items-center justify-center text-center ${
                    ...
                    >
                      <div className="flex items-center justify-center mb-2"> {/* Centering image */}
                        {product.mockup_image_url ? (
                          <img
                            src={product.mockup_image_url}
                            alt={product.name}
                            className="w-24 h-24 object-contain rounded-md" // Increased size for better visibility
                          />
                        ) : (
                          <div className="w-24 h-24 bg-thumbnailPlaceholder rounded-md flex items-center justify-center text-xs text-white">
                            No Image
                          </div>
                        )}
                      </div>
                      <span className="text-lg font-semibold text-gray-800 dark:text-gray-100">{product.name}</span>
