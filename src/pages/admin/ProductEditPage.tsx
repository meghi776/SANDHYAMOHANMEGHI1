...
import { Loader2, XCircle, ArrowLeft, ArrowRight } from 'lucide-react'; // Import ArrowRight
...
const ProductEditPage = () => {
  ...
  const fetchProductsAndCurrent = useCallback(async () => {
    setLoading(true);
    setError(null);

    try {
      // Fetch all products for the current brand and category
      const { data: allProductsData, error: allProductsError } = await supabase
        ...
        .eq('brand_id', brandId)
        .order('name', { ascending: true }); // Order consistently

      if (allProductsError) {
        throw new Error(`Failed to load product list: ${allProductsError.message}`);
      }
      ...
      if (isEditing && productId) {
        ...
        if (productError) {
          throw new Error(`Failed to load product details: ${productError.message}`);
        }

        setProductName(productData.name);
        setProductDescription(productData.description || '');
        setProductPrice(productData.price?.toString() || '');
        setCurrentMockupImageUrl(productData.mockups?.[0]?.image_url || null);
        setCanvasWidth(productData.canvas_width?.toString() || '300');
        setCanvasHeight(productData.canvas_height?.toString() || '600');
        setIsProductDisabled(productData.is_disabled);
        setProductInventory(productData.inventory?.toString() || '0');
        setProductSku(productData.sku || '');
        setMockupX(productData.mockups?.[0]?.mockup_x?.toString() || '0');
        setMockupY(productData.mockups?.[0]?.mockup_y?.toString() || '0');
      ...
      showError(err.message || "Failed to load product data.");
      setError(err.message || "An unexpected error occurred.");
    } finally {
      setLoading(false);
    }
  }, [categoryId, brandId, productId, isEditing]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing) {
        // If editing, stay on the page and re-fetch data to reflect changes
        fetchProductsAndCurrent();
      } else {
        // If adding a new product, navigate to the product list
  ...
  // Determine next/previous product IDs
  ...
  const handleNavigateProduct = (targetProductId: string) => {
    navigate(`/admin/categories/${categoryId}/brands/${brandId}/products/${targetProductId}`);
  };
      ...
      <div className="flex items-center mb-6">
        ...
        {isEditing && (
          <div className="flex space-x-2 ml-auto">
            <Button
              variant="outline"
              onClick={() => previousProduct && handleNavigateProduct(previousProduct.id)}
              disabled={!previousProduct || isSubmitting}
            >
              <ArrowLeft className="mr-2 h-4 w-4" /> Previous
            </Button>
            <Button
              variant="outline"
              onClick={() => nextProduct && handleNavigateProduct(nextProduct.id)}
              disabled={!nextProduct || isSubmitting}
            >
