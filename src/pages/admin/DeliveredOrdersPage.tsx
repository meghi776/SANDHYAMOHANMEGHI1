...
const DeliveredOrdersPage = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      let query = supabase.functions.invoke('get-delivered-orders', {
        headers: {
      ...
      showError(`Failed to load delivered orders: ${err.message}`);
  ...
  const handleDeleteOrder = async (id: string, imageUrl: string | null) => {
    ...
    if (imageUrl) {
      ...
      if (fileName) {
        // Assuming 'order-mockups' is the bucket and 'orders' is the subfolder
  ...
  const handleBulkDownloadDesigns = async () => {
    ...
    if (downloadedCount > 0) {
      zip.generateAsync({ type: "blob" }).then(content => {
        saveAs(content, "delivered_designs.zip");
  ...
  const handleDownloadAllDesigns = async () => {
    ...
    if (downloadedCount > 0) {
      zip.generateAsync({ type: "blob" }).then(content => {
        saveAs(content, "all_delivered_designs.zip");
        showSuccess(`${downloadedCount} designs downloaded.${failedCount > 0 ? ` ${failedCount} failed.` : ''}`); // Report failed count
      });
    } else {
      showError("No designs could be downloaded.");
  ...
  const handleBulkDownloadAddresses = () => {
    ...
    saveAs(blob, `delivered_addresses.csv`);
  ...
  const handleExportAllOrders = () => {
    ...
    const csv = Papa.unparse(dataToExport);
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    saveAs(blob, `delivered_orders_export.csv`);
    showSuccess("All delivered orders exported successfully!");
  };
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-gray-600 dark:text-gray-300">No delivered orders found.</p>
          ) : (
