...
const ShippedOrdersPage = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      let query = supabase.functions.invoke('get-shipped-orders', {
        headers: {
      ...
      showError(`Failed to load shipped orders: ${err.message}`);
  ...
  const handleBulkDownloadAddresses = () => {
    ...
    saveAs(blob, `shipped_addresses.csv`);
  ...
  const handleExportAllOrders = () => {
    ...
    const csv = Papa.unparse(dataToExport);
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    saveAs(blob, `shipped_orders_export.csv`);
    showSuccess("All shipped orders exported successfully!");
  };
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-gray-600 dark:text-gray-300">No shipped orders found.</p>
          ) : (
