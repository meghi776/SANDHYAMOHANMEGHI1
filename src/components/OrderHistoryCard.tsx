...
import { Eye, Image as ImageIcon, XCircle, Edit } from 'lucide-react'; // Import Edit icon
...
const OrderHistoryCard: React.FC<OrderHistoryCardProps> = ({ order, onViewImage, onCancelOrder, onEditOrder }) => {
    ...
    <>
      <Card className="w-full">
        ...
        <CardContent>
          <div className="flex justify-between items-center">
            <div>
              ...
              {order.comment && (
                <p className="text-sm text-red-500 mt-1" title={order.comment}>
        ...
        <CardFooter className="flex flex-wrap justify-between items-center gap-2">
          <Button className="w-full sm:w-auto flex-1" onClick={() => setIsDetailsOpen(true)}>
      ...
      <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
        <DialogContent>
          ...
          <div className="space-y-2 py-4 text-sm">
            ...
            {order.comment && (
              <p><strong>Admin Note:</strong> <span className="text-red-500">{order.comment}</span></p>
            )}
          </div>
        </DialogContent>
      </Dialog>
