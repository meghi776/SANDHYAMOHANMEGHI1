...
serve(async (req) => {
  ...
  try {
    ...
    if (!userId || !phone || !password) {
      return new Response(JSON.stringify({ error: 'User ID, phone, and password are required.' }), {
    ...
    if (profileError || invokerProfile?.role !== 'admin') {
      return new Response(JSON.stringify({ error: 'Forbidden: Only administrators can update user credentials.' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 403,
      });
    }
    ...
    return new Response(JSON.stringify({ message: 'User credentials updated successfully!', user: data.user }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    });
