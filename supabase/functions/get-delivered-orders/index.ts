...
serve(async (req) => {
  console.log("Edge Function: get-delivered-orders invoked."); // Added log
  ...
  try {
    ...
    if (profileError || invokerProfile?.role !== 'admin') {
      ...
      return new Response(JSON.stringify({ error: 'Forbidden: Only administrators can view delivered orders.' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 403,
      });
    }
    ...
    let query = supabaseAdmin
      ...
      .select(`
        ...
        product_id, products (name, printing_width_mm, printing_height_mm), profiles (first_name, last_name), user_id, type, comment, ordered_design_data
      `)
      .eq('status', 'Delivered')
      .order('created_at', { ascending: false });

    console.log("Edge Function: Querying for delivered orders..."); // Added log
    const { data: ordersData, error: ordersError } = await query;

    if (ordersError) {
      console.error("Edge Function: Error fetching delivered orders from DB:", ordersError); // Added log
      throw new Error(`Failed to fetch delivered orders: ${ordersError.message}`);
    }

    console.log(`Edge Function: Found ${ordersData.length} delivered orders.`); // Added log
    ...
    console.log("Edge Function: Returning delivered orders data."); // Added log
    ...
    console.error("Edge Function: Unexpected top-level error in get-delivered-orders:", error);
